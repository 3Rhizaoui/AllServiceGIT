...
export class ArtisansService {
  ...
  async list(raw: any) {
    ...
    const values: any[] = [];
    const where: string[] = ["u.role='artisan'"];
    ...
    // Filter artisans that have at least one matching active service
    if (category || price_min !== undefined || price_max !== undefined) {
      const serviceWhere: string[] = ['s.is_active = TRUE'];
      if (category) {
    ...
    // Geo filter using artisan_service_areas center+radius
    ...
    if (lat !== undefined && lng !== undefined) {
      values.push(lng, lat); // note: ST_MakePoint(lng, lat)
      ...
      // distance: min distance between point and any center
      distanceSelect = `(
      ...
      // coordinates of closest center (so the frontend can place pins)
      latLngSelect = `(
    ...
    if (sort === 'distance') orderBy = 'distance_km ASC NULLS LAST';
    if (sort === 'price') orderBy = 'min_price ASC NULLS LAST';
    if (sort === 'rating') orderBy = 'ap.rating_avg DESC';
  ...
  async getById(id: string) {
    const artisanRes = await this.db.client.query(
      `SELECT
        u.id,
        u.first_name,
        u.last_name,
    ...
    const services = await this.db.client.query(
    ...
    const reviews = await this.db.client.query(
    ...
    const areas = await this.db.client.query(
      `SELECT id, area_name, radius_km,
  ...
  async upsertMyProfile(userId: string, dto: any) {
    ...
    const res = await this.db.client.query(
      `UPDATE artisan_profiles
       ...
       WHERE user_id=$1
       RETURNING *`,
      [userId, business_name ?? null, bio ?? null],
    );
    return res.rows[0];
  }

  async addServiceArea(userId: string, dto: any) {
    ...
    const res = await this.db.client.query(
      `INSERT INTO artisan_service_areas (artisan_user_id, area_name, center, radius_km)
       VALUES ($1, $2, ST_SetSRID(ST_MakePoint($3,$4),4326)::geography, $5)
       RETURNING id, area_name, radius_km, ST_Y(center::geometry) AS lat, ST_X(center::geometry) AS lng`,
      [userId, area_name, lng, lat, radius_km],
    );
    return res.rows[0];
  }
}
